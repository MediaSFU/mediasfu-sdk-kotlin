...
## Choose a media integration

- Use `AttachWebRtcDevice()` when one `IMediaSfuWebRtcDevice` should own both
  publication and playback.
- Use `AttachLocalMediaBackend()` and `AttachRemoteMediaBridge()` when the host
  application has separate capture and renderer adapters.
- Implement the stable `MediaSfuUnity*` ABI when adding another native plugin
  target; follow `Documentation~/NATIVE_PLUGIN_CONTRACT.md` rather than custom
  socket payloads.
- Treat the sample scene as an integration starter. Build product-specific
  moderation, whiteboard, recording, accessibility, and recovery UI around the
  public SDK events and commands.

Before release, verify room lifecycle, bidirectional audio/video, screen share,
recording, moderation, forced exit, and cleanup in the intended Unity Editor and
player targets.
